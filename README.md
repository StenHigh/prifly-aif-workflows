...
Оба пакета объявлены на `WorkflowRevision v4`: набор вердиктов шага закрыт
(`pass`, `fail`, `needs_revision`, `no_work`), и каждый шаговый узел обязан
сказать, куда ведёт каждый из них. Незамаршрутизированный вердикт не отвергается
...
Ревизия запрашивается явной строкой `schema_version: "4"` в `workflow.yaml`.
Выведенная ревизия по построению низшая из возможных и до v4 сама не поднимется.
**Явная `"4"` — обязательство навсегда:** каждый узел, добавленный позже, тоже
...
`impossible_verdicts` не объявлено ни в одном узле, намеренно. Поле читается
только компилятором; если объявленный невозможным вердикт всё-таки придёт, цена
будет ровно та же, что у забытого маршрута. А шаги здесь исполняет
ассистируемый хост: отчёт приходит от исполнителя, и утверждать за него, чего он
не вернёт, автор графа не может. Где есть сомнение — объявляется настоящий исход.
